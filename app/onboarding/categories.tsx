...
export default function CategorySelection() {
  ...
  const [selected, setSelected] = useState<string[]>(['fitness', 'focus', 'discipline']);
    ...
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.subtitle}>
          Select up to {MAX_FREE} categories to track. You can change these later.
        </Text>
        <View style={styles.counter}>
          <Text style={styles.counterText}>{selected.length} / {MAX_FREE} selected</Text>
        </View>
      </View>
      ...
      <View style={[styles.footer, { paddingBottom: insets.bottom + Spacing.md }]}>
        <Pressable
          ...
          onPress={() => {
            if (selected.length > 0) router.push({ pathname: '/onboarding/quests', params: { categories: JSON.stringify(selected) } });
          }}
          disabled={selected.length === 0}
        >
          <Text style={styles.btnText}>Set Up Quests</Text>
